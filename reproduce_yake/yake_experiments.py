...
for name in dnames:
    cm1 = "python3 keyphrase2trec.py --datasetdir ../datasets/{} --results results_for_datasets/".format(name,name)
    os.system(cm1)
    command = "./trec_eval -m set_F -M1000  output/{}.qrel output/{}_results_for_datasets.out > eval_results/extracts_{}.scores".format(name, name, name)
    os.system(command)
...
for j in glob.glob("eval_results/*"):
    try:
            ...
            for line in fx:
                ...
                if len(parts) == 3:
                    try:
                        if parts[2] == ".":
                            continue
                        dfx_rows.append({"score":parts[0],"docID":parts[1],"performance":float(parts[2])})
        ...
        print(es)
pd.DataFrame(results).to_csv("final_eval/final_{}_F_score.tsv".format(num_keywords),sep="\t")
